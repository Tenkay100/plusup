...
async function getShipment(trackingNumber) {
    ...
    const localFallback = async () => {
        const s = getLocalShipments().find(x => (x.tracking_number || '').trim().toUpperCase() === cleanTracking);
        if (s) {
            ...
            s.shipment_history = (getLocalHistory().filter(h => h.shipment_id === s.id)).map(cleanHistoryDesc);
    ...
    try {
        ...
        if (querySnapshot.empty) {
            // Also try exact string in case of mixed case
