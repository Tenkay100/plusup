    ...
    try {
        ...
        if (!h) {
            const defaultHistory = [
                {
                    id: "sample-hist-1",
                    shipment_id: "sample-shipment-1",
                    tracking_number: "AC-12345678",
                    location: "Guangzhou Logistics Hub, China",
                    status: "Shipment Created",
                    description: "Electronic shipping details received and processed.",
                    update_date: new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString()
                },
                {
                    id: "sample-hist-2",
                    shipment_id: "sample-shipment-1",
                    tracking_number: "AC-12345678",
                    location: "Port of Guangzhou, China",
                    status: "Package Received",
                    description: "Cargo loaded and container sealed on MSC VALERIA.",
                ...
                },
                {
                    ...
                    shipment_id: "sample-shipment-1",
                    tracking_number: "AC-12345678",
                    location: "International Waters - Gulf of Guinea",
                    status: "In Transit",
                    description: "Routing update via in transit",
                    update_date: new Date().toISOString()
                }
            ];
            localStorage.setItem('aqua_history', JSON.stringify(defaultHistory));
...
async function getShipment(trackingNumber) {
    ...
    try {
        ...
        // Fetch history by shipment_id and tracking_number
        const historyRef = firestoreDb.collection('shipment_history');
        const historySnapshot = await historyRef.where('shipment_id', '==', shipmentData.id).get();
        let rawHistory = historySnapshot.docs.map(hDoc => ({ id: hDoc.id, ...hDoc.data() }));

        if (rawHistory.length === 0 && shipmentData.tracking_number) {
            // Also check by tracking_number in case shipment_id format differed
            const tnHistSnap = await historyRef.where('tracking_number', '==', shipmentData.tracking_number).get();
            rawHistory = tnHistSnap.docs.map(hDoc => ({ id: hDoc.id, ...hDoc.data() }));
        }

        const localHist = getLocalHistory().filter(h => h.shipment_id === shipmentData.id || h.shipment_id === doc.id || (h.tracking_number && h.tracking_number === shipmentData.tracking_number));
        ...
        let mergedHistory = Array.from(hMap.values());

        // If newly queried on a different device and history list is empty, synthesize initial origin checkpoint so it always shows timeline
        if (mergedHistory.length === 0) {
            mergedHistory = [
                {
                    id: 'init-' + (shipmentData.id || uuidv4()),
                    shipment_id: shipmentData.id,
                    tracking_number: shipmentData.tracking_number,
                    location: shipmentData.origin_country || "Origin Facility",
                    status: shipmentData.status || "Shipment Created",
                    description: shipmentData.status_description || "Electronic shipping details received and processed.",
                    update_date: shipmentData.created_at || new Date().toISOString()
                }
            ];
        }

        shipmentData.shipment_history = mergedHistory;
