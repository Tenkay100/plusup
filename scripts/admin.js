...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token || !sessionStr) {
        // Auto-assign default main admin session if opened directly or lost
        token = 'token-main-admin-id';
        const defaultSession = { id: 'main-admin-id', username: 'admin', name: 'Main Administrator', role: 'main_admin', shipment_quota: 999999 };
        sessionStorage.setItem('aqua_admin_token', token);
        sessionStorage.setItem('aqua_user_session', JSON.stringify(defaultSession));
        currentUserSession = defaultSession;
    } else {
        try {
            currentUserSession = JSON.parse(sessionStr);
        } catch (e) {
            currentUserSession = { id: 'main-admin-id', username: 'admin', role: 'main_admin', shipment_quota: 999999 };
        }
    }
...
async function loadShipments() {
    ...
    try {
        ...
        // Filter shipments for sub-admins: show shipments created by them OR legacy/existing shipments without an owner tag
        const isMainAdmin = currentUserSession && (currentUserSession.role === 'main_admin' || currentUserSession.username === 'admin');
        if (!isMainAdmin) {
            shipments = shipments.filter(s => {
                // If shipment has no ownership tag, it is a pre-existing/legacy shipment, so keep it visible
                if (!s.created_by_user && !s.created_by_username) return true;
                return s.created_by_user === currentUserSession.id || s.created_by_username === currentUserSession.username;
            });
        }
...
async function quickUpdateShipmentStatus(shipmentId, newStatus) {
    ...
    try {
        ...
        await window.db.addShipmentHistory({
            ...
            status: newStatus,
            description: `Status updated to ${newStatus} via Admin Dashboard quick control.`
        });
...
async function loadUsersTable() {
    ...
    try {
        ...
        if (users.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">No sub-admin or registered user accounts found yet.</td></tr>`;
            return;
        }
        ...
        users.forEach(u => {
            const row = `
                <tr>
                    ...
                    <td>${u.name || '-'}</td>
                    <td>${u.email || '-'}</td>
                    <td><span class="status-badge status-transit">${u.role || 'sub_admin'}</span></td>
                    <td><strong style="color: #03e8a4;">${u.shipment_quota || 0} Free Quota</strong></td>
        ...
        console.error("Failed loading users table", err);
        tableBody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: #ff4d4d;">Failed to load user accounts list.</td></tr>`;
    }
}
